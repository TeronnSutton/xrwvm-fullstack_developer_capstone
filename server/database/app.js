...
// If you accept JSON payloads, consider using:
...
// Seed DB at startup (async IIFE). Errors are logged instead of using undefined `res`.
(async () => {
  try {
...
// Keep express.raw if clients send raw JSON; parse Buffer -> string before JSON.parse
app.post('/insert_review', express.raw({ type: '*/*' }), async (req, res) => {
  try {
    const data = JSON.parse(req.body.toString());

    // get highest id; limit(1) to reduce result size and handle empty collection
